/***************************************************************
 * Estimate Camera Move using 2D-2D Feature Match
 **************************************************************/
#include <iostream>
...
void testTriangulation(const Mat& img1, const Mat& img2) {
    ...
    for (size_t i = 0; i < matches.size(); ++i) {
        // points in first camera, and reprojected from 3D points
        ...
        cout << "[1] P_c = " << pt1Cam << ", P_3D = " << pt1Cam3d << ", d = " << points[i].z << endl;

        // points in second camera, and reprojected from 3D points
        Point2d pt2Cam = pixel2cam(keyPoints2[static_cast<size_t>(matches[i].trainIdx)].pt, kCameraMatrix);
        Mat pt2Trans = R * (Mat_<double>(3, 1) << points[i].x, points[i].y, points[i].z) + t;
        pt2Trans /= pt2Trans.at<double>(2, 0);
        cout << "[2] P_c = " << pt2Cam << ", P_3D = " << pt2Trans.t() << endl;
    }
}
...
int main() {
    ...
    // pose estimated 2D-2D
    testPoseEstimate2D2D(img1, img2);
