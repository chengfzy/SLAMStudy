...
int main(int argc, char* argv[]) {
    ...
    LocalParameterization* so3Parameterization = new SO3Parameterization;
    for (auto& c : constraints) {
        ...
        problem.AddResidualBlock(costFunction, lossFunction, itPoseBegin->second.p.data(), itPoseBegin->second.R.data(),
                                 itPoseEnd->second.p.data(), itPoseEnd->second.R.data());
        problem.SetParameterization(itPoseBegin->second.R.data(), so3Parameterization);
        problem.SetParameterization(itPoseEnd->second.R.data(), so3Parameterization);
    }
    ...
    problem.SetParameterBlockConstant(itPoseStart->second.R.data());
