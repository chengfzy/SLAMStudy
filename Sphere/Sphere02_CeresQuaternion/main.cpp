...
int main(int argc, char* argv[]) {
    ...
    // use ProductParameterization, note that the order must be [q, p]. The order of [p, q] will be error
    LocalParameterization* poseParameterization =
        new ProductParameterization(new EigenQuaternionParameterization(), new IdentityParameterization(3));
    for (auto& c : constraints) {
        ...
        problem.SetParameterization(itPoseBegin->second.data(), poseParameterization);
        problem.SetParameterization(itPoseEnd->second.data(), poseParameterization);
    }
